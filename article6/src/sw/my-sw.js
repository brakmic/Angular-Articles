...
var logger = __webpack_require__(1)('ServiceWorker');
var LOCAL_CACHE = 'my-site-cache-v1';
var RUNTIME_CACHE = 'RUNTIME_CACHE';
var urlsToCache = [
    '/',
    'index.html',
    'initial.css',
...
self.addEventListener('install', function (e) {
    ...
    e.waitUntil(self.caches.open(LOCAL_CACHE)
        .then(function (cache) {
        return cache.addAll(_.toArray(urlsToCache));
    }));
});
// The activate handler takes care of cleaning up old caches.
self.addEventListener('activate', function (e) {
    var currentCaches = [LOCAL_CACHE, RUNTIME_CACHE];
    logger.log("Activation.");
    e.waitUntil(self.caches.keys().then(function (cacheNames) {
        return cacheNames.filter(function (cacheName) { return !_.isNil(_.find(currentCaches, cacheName)); });
...
self.addEventListener('fetch', function (e) {
    ...
    if (e.request.url.startsWith(self.location.origin)) {
        e.respondWith(self.caches.match(e.request).then(function (cachedResponse) {
            if (cachedResponse) {
                logger.log("Serving cached response: " + cachedResponse.url);
                return cachedResponse;
            }
            return self.caches.open(RUNTIME_CACHE).then(function (cache) {
                return self.fetch(e.request.clone()).then(function (response) {
